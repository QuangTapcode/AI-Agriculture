...
export function KnowledgeDocumentsTable({ documents }) {
  if (!documents.length) {
    return <div className="rounded-2xl border border-dashed bg-white px-6 py-12 text-center text-gray-500">
    ...
    {documents.map((document) => <article key={document.id} className="min-w-0 rounded-2xl border bg-white p-4 shadow-sm">
      <div className="flex min-w-0 flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 break-words text-sm text-gray-500 [overflow-wrap:anywhere]">{document.source_name}</p>
        </div>
      ...
      <dl className="mt-4 grid grid-cols-2 gap-3 border-t pt-4 text-sm sm:grid-cols-3 xl:grid-cols-6">
        <div><dt className="text-xs text-gray-400">Chủ đề</dt><dd className="mt-1 text-gray-700">{document.crop || '—'}</dd></div>
        <div><dt className="text-xs text-gray-400">Khu vực</dt><dd className="mt-1 text-gray-700">{document.region || '—'}</dd></div>
        <div><dt className="text-xs text-gray-400">Phiên bản</dt><dd className="mt-1 text-gray-700">v{document.version}</dd></div>
        <div><dt className="text-xs text-gray-400">Chất lượng</dt><dd className="mt-1 text-gray-700">{document.quality_score == null ? '—' : `${Math.round(document.quality_score * 100)}%`}</dd></div>
        <div><dt className="text-xs text-gray-400">Embedding</dt><dd className="mt-1 text-gray-700">{document.chunks} đoạn</dd></div>
        <div><dt className="text-xs text-gray-400">Ngày nạp</dt><dd className="mt-1 text-gray-700">{formatDate(document.fetched_at)}</dd></div>
      </dl>
    </article>)}
  </div>;
}
  ...
  return <div className="mx-auto w-full max-w-7xl space-y-6">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
      <div>
        ...
        <p className="mt-2 text-sm text-gray-500">Theo dõi tài liệu mới, kết quả kiểm tra chất lượng và trạng thái lập chỉ mục.</p>
      </div>
    ...
    <section className="grid grid-cols-2 gap-3 lg:grid-cols-4">
      <div className="rounded-2xl border bg-white p-4"><p className="text-sm text-gray-500">Đã duyệt</p><strong className="mt-2 block text-2xl">{summary.approved ?? '—'}</strong></div>
      <div className="rounded-2xl border bg-white p-4"><p className="text-sm text-gray-500">Đã lập chỉ mục</p><strong className="mt-2 block text-2xl">{summary.indexed_documents ?? '—'}</strong></div>
      <div className="rounded-2xl border bg-white p-4"><p className="text-sm text-gray-500">Đoạn embedding</p><strong className="mt-2 block text-2xl">{summary.indexed_chunks ?? '—'}</strong></div>
      <div className="rounded-2xl border bg-white p-4"><p className="text-sm text-gray-500">Mới trong lần chạy</p><strong className="mt-2 block text-2xl">{newCount}</strong></div>
    </section>

    <section className="flex flex-col gap-3 rounded-2xl border bg-white p-4 md:flex-row md:items-center">
      <div className="relative min-w-0 flex-1"><Search className="absolute left-3 top-2.5 text-gray-400" size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Tìm tiêu đề, nguồn, cây trồng hoặc khu vực…" className="w-full rounded-xl border py-2 pl-10 pr-3 text-sm" /></div>
      <select value={status} onChange={(event) => setStatus(event.target.value)} className="rounded-xl border bg-white px-3 py-2 text-sm">
      ...
      {summary.last_run_started_at && <span className="inline-flex shrink-0 items-center gap-1 text-xs text-gray-500"><Clock3 size={14} />Lần chạy: {formatDate(summary.last_run_started_at)}</span>}
    </section>

    {error && <div role="alert" className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{error}</div>}
    {loading && !catalogue.documents.length
      ? <div role="status" className="rounded-2xl border bg-white p-10 text-center text-gray-500">Đang tải danh sách tài liệu…</div>
