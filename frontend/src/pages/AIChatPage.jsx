...
export default function AIChatPage() {
  ...
  const loadHistory = useCallback(async (offset = 0) => {
    ...
    try {
      ...
      setHistory((previous) => offset ? [...previous, ...data.history.filter((item) => !previous.some((p) => p.id === item.id))] : data.history);
  ...
  const refreshDocuments = async () => {
    ...
    try {
      ...
      if (version === epoch.current) setDocuments(data.documents);
  ...
  const sidebar = <>
    ...
    {isAuthenticated ? <>
      <div className="relative"><Search size={16} className="absolute left-3 top-3 text-gray-400" /><input aria-label="Tìm trong lịch sử chat" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Tìm nội dung hội thoại…" className="w-full rounded-lg border py-2 pl-9 pr-3 text-sm" /></div>
      <div className="my-3 flex justify-between text-xs text-gray-500"><button onClick={() => loadHistory()} disabled={historyLoading}>Tải lại</button><button disabled={locked || !history.length} onClick={() => setConfirmDelete({ kind: 'all' })} className="text-red-600 disabled:opacity-40">Xóa lịch sử</button></div>
      <div className="space-y-2">{history.map((item, index) => <div key={item.id}>
        {(index === 0 || dateLabel(history[index - 1].created_at) !== dateLabel(item.created_at)) && <p className="mb-2 mt-4 text-xs font-semibold text-gray-500">{dateLabel(item.created_at)}</p>}
        <div className={`flex rounded-xl border ${sessionId === item.id ? 'border-green-300 bg-green-50' : 'border-transparent hover:bg-gray-50'}`}>
          <button disabled={locked} onClick={() => openConversation(item)} className="min-w-0 flex-1 p-3 text-left"><p className="truncate text-sm font-semibold">{item.user_message}</p><p className="mt-1 truncate text-xs text-gray-500">{item.ai_response}</p><p className="mt-2 text-xs text-gray-400">{item.turn_count} lượt · {timeLabel(item.created_at)}</p></button>
          <button disabled={locked} onClick={() => setConfirmDelete({ kind: 'conversation', id: item.id })} aria-label={`Xóa hội thoại ${item.user_message}`} className="self-start p-2 text-gray-400 hover:text-red-600"><Trash2 size={15} /></button>
        </div>
      </div>)}</div>
      {historyLoading && <p role="status" className="py-4 text-sm text-gray-500">Đang tải lịch sử…</p>}
      {!historyLoading && !history.length && <p className="py-6 text-sm text-gray-500">{query ? 'Không tìm thấy hội thoại phù hợp.' : 'Hội thoại của bạn sẽ xuất hiện ở đây.'}</p>}
  ...
  return <div className="flex h-[calc(100dvh-8rem)] min-h-[560px] max-w-full gap-4 overflow-x-hidden">
    ...
    <section className="flex min-w-0 flex-1 flex-col overflow-hidden rounded-2xl border bg-white">
      <header className="flex items-center gap-3 border-b p-4">
        ...
        <div className="min-w-0 flex-1"><h1 className="font-bold text-gray-900">Trợ lý nông nghiệp</h1><p className="text-xs text-gray-500">Tra cứu tài liệu · Ghi nhớ hội thoại</p></div>
        <button onClick={() => { setLibraryOpen(!libraryOpen); if (!libraryOpen) refreshLibrary(); }} className="flex items-center gap-2 rounded-lg border p-2 text-sm text-green-800"><BookOpen size={18} /><span className="hidden sm:inline">Kho tài liệu</span></button>
      </header>
      ...
      {libraryOpen && <div className="max-h-96 shrink-0 overflow-y-auto border-b bg-gray-50 p-4">
        ...
        {isAuthenticated && <div className="mt-3 rounded-xl border border-green-200 bg-white p-3 text-xs">
          ...
          {knowledgeLoading && !knowledgeStatus && <p role="status" className="mt-2 text-gray-500">Đang đọc trạng thái…</p>}
          {knowledgeStatus && <>
            <div className="mt-2 flex flex-wrap items-center gap-2">
              ...
              <span className="text-gray-500">Lịch chạy: {String(knowledgeStatus.schedule_hour).padStart(2, '0')}:00 mỗi ngày</span>
            </div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
              ...
              <div className="rounded-lg bg-gray-50 p-2"><strong className="block text-base text-gray-900">{knowledgeStatus.last_run?.records_fetched ?? 0}</strong>Tài liệu phát hiện</div>
              <div className="rounded-lg bg-gray-50 p-2"><strong className="block text-base text-gray-900">{knowledgeStatus.last_run?.records_saved ?? 0}</strong>Tài liệu mới</div>
        ...
        <p className="my-2 text-xs leading-5 text-gray-500">Nạp sách kỹ thuật, hướng dẫn canh tác hoặc dữ liệu mùa vụ để trợ lý tra cứu. PDF có văn bản, TXT, Markdown · tối đa 10 MB/tệp.</p>
        {isAuthenticated ? <>
          ...
          {!documents.length && <p className="mt-3 text-xs text-gray-500">Chưa có tài liệu. Thêm tài liệu để bắt đầu đối chiếu nguồn.</p>}
          {documents.map((doc) => <div key={doc.id} className="mt-2 flex items-center gap-2 rounded-lg border bg-white p-2 text-xs"><FileText size={16} className="shrink-0 text-green-700" /><span className="min-w-0 flex-1 truncate">{doc.name} · {doc.chunks} đoạn</span><button disabled={locked} onClick={() => setConfirmDelete({ kind: 'document', id: doc.id })} aria-label={`Xóa tài liệu ${doc.name}`} className="p-1 text-gray-500 hover:text-red-600"><Trash2 size={15} /></button></div>)}
